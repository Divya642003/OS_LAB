...
int main()
{
    ...
    while(process_completed<n)
    {
        ...
        if(a[max_index].res_time==-1)
        {
            a[max_index].res_time=time-a[max_index].arrival_time;
        }

        time++;
        a[max_index].rem_time--;

        if(a[max_index].rem_time==0)
        {
            a[max_index].comp_time=time;
            process_completed++;
        }
    }
